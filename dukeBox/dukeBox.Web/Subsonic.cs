...
namespace dukeBox
{
    ...
    public static class SongQueue
    {
        ...
        public static void AddSongToQueue(Song song)
        {
            ...
            context.Clients.All.addSongToQueue(song);
        }
    ...
    public class SubsonicItem : IEquatable<SubsonicItem>
    {
        ...
        bool IEquatable<SubsonicItem>.Equals(SubsonicItem other)
        {
    ...
    public class Song : SubsonicItem
    {
        ...
        public Song(string title, string artist, string album, string id)
        {
            ...
            this.downloadUrl = Subsonic.BuildDirectURL("download", new Dictionary<string, string> { { "id", this.id } });
    ...
    public static class Subsonic
    {
        ...
        public static string appName = "dukeBox";

        // Min version of the REST API implemented
        private static string apiVersion = "1.10.2";
        ...
        public static List<SubsonicItem> Search(string query, bool includeSongs = true, bool includeArtists = true, bool includeAlbums = true)
        {
            Dictionary<string, string> parameters = new Dictionary<string, string>();
            ...
            if (myXML.ChildNodes[1].Name == "subsonic-response")
            {
                if (myXML.ChildNodes[1].FirstChild.Name.Contains("searchResult"))
                {
                    var nodes = myXML.ChildNodes[1].FirstChild.ChildNodes;
                    foreach (XmlNode node in nodes)
                    {
                        if (!includeSongs && node.Name == "song")
                        {
                            continue;
                        }

                        if (!includeArtists && node.Name == "artist")
                        {
                            continue;
                        }

                        if (!includeAlbums && node.Name == "album")
                        {
                            continue;
                        }

                        bool isDir = node.Attributes["isDir"] != null && bool.Parse(node.Attributes["isDir"].Value);
                        ...
                        if (isDir)
                            ...
                            theItem = new Song(title, artist, album, theId) { duration = duration };
